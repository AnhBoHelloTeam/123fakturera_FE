...
function Contact({ language, setLanguage }) {
  ...
  useEffect(() => {
    const fetchContact = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/contact?language=${language}`);
        setTitle(response.data.title || 'Contact Us');
        setButton(response.data.button || 'Go to Home');
        setContext(response.data.context || '<p>Contact 123 Fakturera for support or inquiries.</p>');
        setLoading(false);
      } catch (err) {
